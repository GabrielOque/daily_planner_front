...
const CreateAndEditEvent = ({
  ...
  const handleEditEvent = async () => {
    ...
    try {
      ...
      await axios.put(
        `${NEXT_PUBLIC_API_URL}/event/update-event/${selectedEvent.id}`,
        eventData
      );
      setLoadingCreateEvent(false);
  ...
  const deleteEvent = async () => {
    ...
    try {
      await axios.delete(
        `${NEXT_PUBLIC_API_URL}/event/delete-event/${selectedEvent.id}`
      );
      setLoadingCreateEvent(false);
  ...
  const isValidDateTimeRange = useMemo(() => {
  ...
  const isDiferent = useMemo(() => {
    return (
      ...
      participants.length === selectedEvent?.participants.length &&
      participants.every((participant) =>
        selectedEvent?.participants.includes(participant)
      )
    );
  }, [name, description, startDate, startTime, endTime, participants]);
            ...
            <div className="flex mt-4 gap-x-2 justify-center">
              ...
              {selectedEvent && selectedEvent?.coordinator === user.email && (
                <div className="w-24">
                  <Button
                    loading={loadingCreateEvent}
                    disabled={isDiferent || !isValidDateTimeRange}
                    paddingY="py-1"
                    paddingX={"px-4"}
                    background="bg-primary"
                    fontSize="text-sm"
